...
namespace FRFuel
{
    public class FRFuel : BaseScript
    {
...
#if DEBUG
                Debug.WriteLine(GasStations.pumptypes[i]);
#endif
        ...
        public bool IsVehicleNearAnyFuelTruck(Vehicle vehicle)
        {

            var fuelTankPos = GetVehicleTankPos(vehicle);
            Vector3 vehiclePosition = Game.PlayerPed.CurrentVehicle.Position;

            var closestVehicle = World.GetClosest(Game.PlayerPed.Position, World.GetAllVehicles().Where(i => i.Model == GetHashKey("fueltruck")).ToArray());

            if (closestVehicle != null)
            {

                        if (closestVehicle.Model == new Model("fueltruck") && (World.GetDistance(Game.PlayerPed.Position, closestVehicle.Position) < 12f))
                        {
                            return true;
                        }
                        else
        ...
        public void ConsumeFuel(Vehicle vehicle)
        {
            ...
            if ( (
              ...
              // And near any pump
              IsVehicleNearAnyPump(vehicle)) || ((IsVehicleNearAnyFuelTruck(vehicle))))
            {
#if MANUAL_ENGINE_CUTOFF
